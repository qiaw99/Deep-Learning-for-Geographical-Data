...
BASE_DIR = os.path.dirname(os.path.abspath("~/Research/Object Detection/VoteNet/ISPRS"))
...
DC = ISPRSDatasetConfig() # dataset specific config
MAX_NUM_OBJ = 80 # maximum number of objects allowed per scene
MEAN_COLOR_RGB = np.array([0.5,0.5,0.5]) # sunrgbd color is in 0~1

class ISPRSDetectionVotesDataset(Dataset):
...
# raw data path with nirgends im Projekt verwendet
#        self.raw_data_path = os.path.join(ROOT_DIR, 'sunrgbd/sunrgbd_trainval')

        self.data_path = os.path.expanduser('~/coursematerial/GIS/ISPRS/VoteNet/Patches100k')
    
        self.scan_names = sorted(list(set([os.path.basename(x)[0:10] \
    ...
    def __getitem__(self, idx):
        ...
        Returns a dict with following keys:
            ...
            # MK
            patch_center: (3) x, y, z, of the center of the patch
        """
        ...
        # MK
        # translate everything by center point
        center = (0.5 * (np.min(point_cloud[0:3], axis=0) + np.max(point_cloud[0:3], axis=0)))[np.newaxis, :]
        point_cloud[:, 0:3] = point_cloud[:, 0:3] - center
        bboxes[:, 0:3] = bboxes[:, 0:3] - center
        ...
        if self.augment:
            ...
            rot_mat = ISPRS_utils.rotz(rot_angle)
        ...
        for i in range(bboxes.shape[0]):
            bbox = bboxes[i]
            corners_3d = ISPRS_utils.my_compute_box_3d(bbox[0:3], bbox[3:6], bbox[6])
        ...
        # MK
        ret_dict['patch_center'] = center
        return ret_dict

def viz_votes(pc, point_votes, point_votes_mask, s_id=""):
    ...
    pc_util.write_ply(pc_obj, s_id + 'pc_obj.ply')
    pc_util.write_ply(pc_obj_voted1, s_id + 'pc_obj_voted1.ply')
    pc_util.write_ply(pc_obj_voted2, s_id + 'pc_obj_voted2.ply')
    pc_util.write_ply(pc_obj_voted3, s_id + 'pc_obj_voted3.ply')

def viz_obb(pc, label, mask, angle_classes, angle_residuals,
    size_classes, size_residuals, s_id = ""):
    ...
    pc_util.write_oriented_bbox(oriented_boxes, s_id + 'gt_obbs.ply')
    pc_util.write_ply(label[mask==1,:], s_id + 'gt_centroids.ply')

def get_sem_cls_statistics():
    """ Compute number of objects for each semantic class """
    d = ISPRSDetectionVotesDataset(use_height=True, use_color=True, use_v1=True, augment=True)
    sem_cls_cnt = {}
...
def write_ISPRSDetectionVotesDataset(i_sample=200):
    d = ISPRSDetectionVotesDataset(use_height=True, use_color=False, use_v1=False, augment=False)
    sample = d[i_sample]
    s_id = f'{i_sample:04d}'
#    print(sample['vote_label'].shape, sample['vote_label_mask'].shape)
    pc_util.write_ply(sample['point_clouds'], s_id + 'pc.ply')
    viz_votes(sample['point_clouds'], sample['vote_label'], sample['vote_label_mask'], s_id)
    viz_obb(sample['point_clouds'], sample['center_label'], sample['box_label_mask'],
        ...
        sample['size_class_label'], sample['size_residual_label'], s_id)    
